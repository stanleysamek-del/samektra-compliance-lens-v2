...
- POSITIVE FORCING RULE — you MUST emit at least one finding when ANY of the following is visible in an above-ceiling photo:
  ...
  * An open hole / oversized opening in a wall, regardless of penetrant. Same severity scaling.
...
- Large unprotected opening (gap > 1 in. with no penetrant filling it, or oversized hole with debris/sleeve only): if the wall is CONFIRMED rated → High, Fire, NFPA 101 §8.3.5.1. If suspected-but-unconfirmed → Medium with conditional language. If confirmed non-rated → Low advisory (best practice to seal, but not a code violation). Title: "Unprotected through-opening — if rated assembly, NFPA 101 §8.3.5.1 violation".
